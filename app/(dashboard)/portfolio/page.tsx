        ...
        <div className="flex items-center justify-center flex-col ">
          {projects.map((item) => (
            <div
              ...
              className="w-[300px] md:w-[800px] h-[300px] p-4 relative bg-[#444] rounded mt-10 flex flex-col justify-between gap-4"
            >
              <header className=" p-2">
                <h2 className="text-2xl text-6xl font-bold">{item.title}</h2>
                <p className="mt-3 text-gray-500 hover:text-gray-200">
                  {item.desription}
                </p>
              </header>
              <span className="absolute right-0 bg-[#222] p-2 -top-5 font-bold rounded drop-shadow-lg">
                {item.type}
              </span>
              <nav>
                <Link
                  href={`/portfolio/projects${item.link}`}
                  className="p-2 font-bold text-emerald-500 bg-[#222] rounded hover:bg-[#444]"
                >
                  {item.link.slice(0, 8)}
                </Link>
              </nav>
